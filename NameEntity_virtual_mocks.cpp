...
namespace NameEntity_virtual_mocks {
  // As before...
  ...
  void NameEntity(IDatabase& db, IUserIO& io) {
    auto const full_name = GetFullNameFromUser(db, io);

    string short_name = DefaultShortName(full_name);
    if (db.IsShortNameUsed(short_name)) {
      short_name = GetShortNameFromUser(db, io);
    }

    db.AddNames(full_name, short_name);
  }

  // * writing tests for NameEntity is now possible but
  // really awkward
  // * really just want to test that it calls its helpers
  // correctly, not what the helpers themselves do (they're
  // already tested)
